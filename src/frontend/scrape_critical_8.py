...
def main():
    input_file = 'Supabase Snippet Untitled query (57).csv'
    
    resources = []
    with open(input_file, 'r', encoding='utf-8') as f:
        resources = list(csv.DictReader(f))
    
    print(f"📥 Loaded {len(resources)} CRITICAL resources")
    print("⚠️  These are missing phone/website - high priority!")
    
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_file = f'scrape_results_critical_{timestamp}.csv'
    
    print(f"\n🚀 Starting targeted scrape...")
    print("=" * 60)
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for i, resource in enumerate(resources, 1):
            print(f"[{i}/{len(resources)}] {resource.get('name', 'Unknown')[:50]}...", end=' ', flush=True)
    ...
    print(f"\n📄 Results: {output_file}")
    print(f"\n💰 API Costs: {len(resources) * 2} API calls (${len(resources) * 2 * 0.017:.2f})")
