...
const extractSlugFromPath = (): string | undefined => {
  ...
  if (segments.length >= 3 && segments[0] === 'resources' && segments[1] === 'insurances') {
    return segments[2];
  }
  return undefined;
};
...
const InsuranceDetail: React.FC<InsuranceDetailProps> = ({ onNavigate, slug: initialSlug }) => {
  ...
  useEffect(() => {
    ...
    loadResource('insurances', targetSlug)
      .then((data) => {
        if (isMounted) {
  ...
  const linkKey = slug ? `insurances/${slug}` : undefined;
  ...
  const isValidInsurance = resource && resource.category === 'insurances';
  ...
  if (!isValidInsurance) {
    return (
      <div className="mx-auto max-w-3xl rounded-3xl bg-white/90 p-10 text-center shadow-lg ring-1 ring-slate-100 dark:bg-slate-900/80 dark:ring-slate-800">
        <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-blue-100 text-blue-600 dark:bg-blue-900/40 dark:text-sky-200">
        ...
        <div className="mt-6 flex justify-center gap-4">
          <button
            type="button"
            onClick={() => onNavigate('/resources/insurances')}
    ...
    <article className="mx-auto flex w-full max-w-3xl flex-col gap-8">
      <header className="rounded-3xl bg-white/95 p-8 shadow-lg ring-1 ring-slate-100 dark:bg-slate-900/80 dark:ring-slate-800">
        ...
        <nav aria-label="Breadcrumb" className="text-sm text-slate-500 dark:text-slate-400">
          Home <span aria-hidden="true">&gt;</span> Resources <span aria-hidden="true">&gt;</span> Insurance & Funding{' '}
      ...
      <div className="rounded-3xl bg-white/95 p-8 shadow-lg ring-1 ring-slate-100 dark:bg-slate-900/80 dark:ring-slate-800">
        <p className="text-sm font-medium uppercase tracking-wide text-blue-600 dark:text-sky-300">Coverage Snapshot</p>
        ...
        {validLinks.length ? (
          <div className="mt-8 rounded-2xl border border-blue-100 bg-white p-6 shadow-sm dark:border-blue-900/60 dark:bg-slate-900/70">
            <h2 className="text-lg font-semibold text-blue-700 dark:text-sky-200">Learn More</h2>
              ...
              {validLinks.map((link) => (
                <li key={link.url}>
                  <a
                    ...
                    className="inline-flex items-start gap-2 text-blue-700 underline-offset-2 hover:underline dark:text-sky-200"
                  >
