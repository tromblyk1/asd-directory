...
def main():
    ...
    print(f"📥 Loaded {len(resources)} resources from {input_file}")
    print(f"⏱️  Estimated time: {len(resources) * 1.1 / 60:.1f} minutes")
    
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_file = f'scrape_results_final_{timestamp}.csv'
